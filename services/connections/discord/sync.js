...
exports.load = async (IP, Req, Res) => {
  ...
  if (getAccount.Connections && getAccount.Connections.Discord && getAccount.Connections.Discord.ID !== "") {
    if ((Number(getAccount.Connections.Discord.LastUpdate) + (60 * 30)) > (Date.now() / 1000)) return Res.send({ status: 400, message: 'Você vinculou a sua conta ou sincronizou os dados a pouco tempo, aguarde para sincronizar novamente.' }); 
